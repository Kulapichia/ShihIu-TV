...
export async function speedTestAllSources(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `   🔒 已屏蔽: ${blockedSourceKeys.length} 个 (速度慢或不可用)\n`
    );
...
export default function SourceAvailabilityChecker() {
  ...
  useEffect(() => {
    ...
    const performSpeedTest = async () => {
      ...
      if (typeof window !== 'undefined') {
        try {
          ...
          console.warn('[SourceChecker] 读取测速缓存失败', e);
        }
      }
      ...
      try {
        ...
        if (typeof window !== 'undefined') {
          try {
            ...
            console.warn('[SourceChecker] 保存测速时间失败', e);
          }
        }
      } finally {
        // 无论成功失败都要解除锁定
    ...
    // 延迟5秒执行，避免影响主应用加载性能
    const timeoutId = setTimeout(performSpeedTest, 5000);
