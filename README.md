...
#### 💬 弹幕生态系统
- **多源弹幕集成**：
  - **第三方 API**：聚合腾讯、爱奇艺、优酷、B站等主流平台弹幕，智能内容匹配防预告。
  - **本地文件支持**：支持上传本地 XML 弹幕文件，提供**单文件**和**批量上传**模式（批量模式下，文件按名称排序后自动匹配剧集）。
- **智能过滤与优化**：
  - **弹幕智能合并**：自动合并指定时间窗口内的重复弹幕，合并后显示数量角标（如 ×₅），并根据合并次数智能增大字号，提升高热度片段观感。
  - **性能优化**：基于设备性能分级渲染、Web Worker 加速、硬件加速、分段加载。
- **全功能配置系统**：
  - **基础调节**：字号、速度、透明度、显示区域、防重叠、按类型屏蔽。
  - **高级调节**：支持**时间对轴**手动校准弹幕。
- **数据管理**：
  - **弹幕导出**：支持将当前加载的弹幕一键下载为 XML 文件，方便备份与分享。
  - **智能缓存**：localStorage 持久化，30 分钟缓存，自动清理过期数据。
- **弹幕交互**：
...
### 🛠️ 技术优化
- ✅ **视频源优选系统**：启动时或手动触发对所有视频源进行速度测试，智能排序并优先启用最快的源，测速结果缓存24小时，提升加载速度。
